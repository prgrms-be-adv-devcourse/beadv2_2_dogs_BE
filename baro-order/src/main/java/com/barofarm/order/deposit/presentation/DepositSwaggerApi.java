...
public interface DepositSwaggerApi {
    ...
    @Operation(
        ...
        description = "예치금 충전을 위한 충전 요청(DepositCharge)을 생성한다. 실제 충전은 Toss 결제 승인(confirmDeposit) 이후 반영된다."
    )
    @ApiResponses({
        @ApiResponse(
            responseCode = "200",
    ...
    @Operation(
        summary = "예치금 결제 환불",
        description = "예치금 결제로 처리된 주문을 환불한다. 결제(Payment)를 REFUNDED로 변경하고 예치금을 복구한다. 이미 환불된 경우 멱등 처리로 0원 환불로 응답할 수 있다."
    )
    @ApiResponses({
        @ApiResponse(
            responseCode = "200",
        ...
        @ApiResponse(
            responseCode = "404",
            description = "예치금 계정/결제 내역이 존재하지 않음 (DEPOSIT_NOT_FOUND, DEPOSIT_PAYMENT_NOT_FOUND)",
            content = @Content(mediaType = "application/json")
        ),
        @ApiResponse(
            responseCode = "409",
            description = "환불 불가 상태 (예: 정책에 따라 status 체크 시)",
            content = @Content(mediaType = "application/json")
        )
    })
    @PostMapping("/refund")
