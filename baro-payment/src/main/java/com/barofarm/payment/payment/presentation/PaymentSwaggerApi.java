...
public interface PaymentSwaggerApi {
    ...
    @Operation(
        summary = "Toss 결제 승인",
        description = "Toss Payments 결제 승인 API를 호출하여 결제를 확정하고, 주문의 결제 완료 상태로 변경합니다."
    )
    @ApiResponses({
        @ApiResponse(
            responseCode = "200",
        ...
        @ApiResponse(
            responseCode = "401",
            description = "유효하지 않은 Toss Secret Key (INVALID_SECRET_KEY)",
            content = @Content(mediaType = "application/json")
        ),
        @ApiResponse(
            responseCode = "404",
    ...
    @Operation(
        summary = "Toss 결제 취소(환불)",
        description = "Toss Payments 결제 취소(환불) API를 호출하여 결제를 취소하고, 주문 상태를 취소 상태로 변경합니다."
    )
    @ApiResponses({
        @ApiResponse(
            responseCode = "200",
        ...
        @ApiResponse(
            responseCode = "401",
            description = "Toss 인증 실패 / Secret Key 오류 (INVALID_SECRET_KEY, TOSS_PAYMENT_UNAUTHORIZED)",
            content = @Content(mediaType = "application/json")
        ),
        @ApiResponse(
            responseCode = "404",
        ...
        @ApiResponse(
            responseCode = "409",
            description = "이미 취소되었거나 취소 불가 상태 (TOSS_PAYMENT_CONFLICT)",
            content = @Content(mediaType = "application/json")
        ),
        @ApiResponse(
            responseCode = "500",
            description = "Toss 서버 오류로 인한 취소 실패 (TOSS_PAYMENT_SERVER_ERROR, TOSS_PAYMENT_CANCEL_FAILED)",
            content = @Content(mediaType = "application/json")
        )
    })
    @PostMapping("/refund")
    ...
    @Operation(
        ...
        description = "Toss Payments 결제 승인 API를 호출하여 예치금 충전 결제를 확정하고, 예치금 및 결제 이력을 기록합니다."
    )
    @ApiResponses({
        @ApiResponse(
            responseCode = "200",
            description = "예치금 충전 결제 승인 성공",
            content = @Content(mediaType = "application/json")
        ),
        @ApiResponse(
            responseCode = "400",
        ...
        @ApiResponse(
            responseCode = "409",
            description = "이미 처리되었거나 충전 불가 상태 (DEPOSIT_CHARGE_INVALID_STATUS, TOSS_PAYMENT_CONFLICT)",
            content = @Content(mediaType = "application/json")
        ),
        @ApiResponse(
            responseCode = "500",
            description = "Toss 서버 오류로 인한 승인 실패 (TOSS_PAYMENT_SERVER_ERROR, TOSS_PAYMENT_CONFIRM_FAILED)",
            content = @Content(mediaType = "application/json")
        )
    })
