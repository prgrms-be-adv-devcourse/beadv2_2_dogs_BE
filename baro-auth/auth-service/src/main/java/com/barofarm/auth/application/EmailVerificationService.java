...
public class EmailVerificationService {
    ...
    // Spring이 사용하는 생성자
    @Autowired
    public EmailVerificationService(EmailVerificationJpaRepository repository,
                                    EmailCodeSender emailCodeSender, Clock clock) {
    ...
    public void verifyCode(String email, String code) {
        EmailVerification verification = repository
            .findByEmailAndCodeAndVerifiedIsFalse(email, code)
            .orElseThrow(() -> new BusinessException(
                HttpStatus.BAD_REQUEST,
                "이메일 인증 코드를 찾을 수 없습니다."
            ));
    ...
    /**
     * 최종 회원가입 전에 호출 -> 인정 완료 여부 확인 + 레코드 제거
     * */

    public void assertEmailVerifiedAndConsume(String email){
        EmailVerification latest = repository
            .findTopByEmailOrderByCreatedAtDesc(email)
            .orElseThrow(() -> new BusinessException(
                HttpStatus.NOT_FOUND, "이메일 인증 이력이 없습니다."
            )); // 완료되면 바로바로 삭제 해놔서
